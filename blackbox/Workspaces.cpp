...
{
	tasklist* tl = (tasklist*)assoc(taskList, hwnd);
...
void Workspaces::WindowActivated(tasklist* tl, HWND hwnd)
{
	struct tasklist* tl2;
	HWND prev_fg_window;
...
void Workspaces::DestoryWindow(tasklist* tl, HWND hwnd)
{
...
void Workspaces::TaskProc(WPARAM wParam, HWND hwnd)
{
	...
	switch (wParam & 0x7FFF)
	{
		...
		case HSHELL_WINDOWCREATED: // 1
			...
			// Clodio: The IsAppWindow function is needed here or you would get junk windows cluttering the tasklist
			{
				if (IsCheckAppWindow(hwnd, false))
				{
					AddTask(hwnd);
					workspaces_set_desk();
				}
		...
		case HSHELL_REDRAW: // 6
			if (tl)
			{
				...
				if (false == tl->active && GetForegroundWindow() == GetLastActivePopup(hwnd))
				{
					WindowActivated(tl, hwnd);
					break;
				}
