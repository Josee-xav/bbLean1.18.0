...
BOOL(WINAPI* pSetWindowCompositionAttribute)(HWND, WINCOMPATTRDATA*);

BOOL(WINAPI* pIsShellManagedWindow)(HWND);
BOOL(WINAPI* pIsShellFrameWindow)(HWND);

HWND(WINAPI* pHungWindowFromGhostWindow)(HWND);
HWND(WINAPI* pGhostWindowFromHungWindow)(HWND);
		...
		if(IsWindowVisible(v5)
		   ...
		   && !pGhostWindowFromHungWindow(v5)) {
			for(i = v5; ; i = v16) {
				...
				if(!IsWindowVisible(v16) || pGhostWindowFromHungWindow(v16))
					break;
...
bool WindowQueryHelper::isAppWindow(HWND hwnd, bool checkWin10) {
	...
	//  ((BOOL(WINAPI *) (HWND, int))pMSWinShutdown)(BBhwnd, 0);
	if(((((BOOL(WINAPI*) (HWND))pIsShellFrameWindow)(hwnd) && ! ((BOOL(WINAPI*) (HWND))pGhostWindowFromHungWindow)(hwnd)))) {
	...
	if(((BOOL(WINAPI*) (HWND))pIsShellManagedWindow)(hwnd) && !ShouldTreatShellManagedWindowAsNotShellManaged(hwnd)) {
		return FALSE;
	}
...
void winEventProc(
	...
	if((event == EVENT_OBJECT_UNCLOAKED) && hwnd && idObject == OBJID_WINDOW && pIsShellFrameWindow(hwnd) && !pGhostWindowFromHungWindow(hwnd)) {
		SendMessage(FindWindowW((L"BlackboxClass"), NULL), g_WM_ShellHook, WM_USER + 44, (LPARAM)hwnd); // sends the UWP message to the SHELLHOOK proc wit WM_WINDOWCREATED
	}
...
BOOL WindowQueryHelper::initWinHelper() {
	...
	if(!SetWinEventHook(
		...
		OutputDebugString("Failed to hook to win EVENTS\n");
		return false;
	}
